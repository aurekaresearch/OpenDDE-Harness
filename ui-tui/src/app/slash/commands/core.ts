...
export const coreCommands: SlashCommand[] = [
...

  {
    ...
    // and breaks every opendde that reads the shared file). Session-scoped.
  ...
  {
    help: 'copy the selection or a reply',
    name: 'copy',
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try OPENDDE_HARNESS_TUI_FORCE_OSC52=1 to force the escape sequence; OPENDDE_HARNESS_TUI_DEBUG_CLIPBOARD=1 for details'
          )
        }
      }
  ...
  {
    help: 'view the current transcript (your messages and the replies)',
    name: 'history',
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `OpenDDE Harness #${i + 1}`
