...
export function useMainApp(gw: GatewayClient, rpcClient?: ChatStreamRpcClient) {
  ...
  // Fullscreen mouse tracking owns drag selections, so the outer terminal
  // cannot copy them normally. macOS terminals often do not forward Cmd+C;
  // remote sessions additionally need OSC 52 to reach the local clipboard.
  // Copy a stable TUI selection automatically while keeping it highlighted.
  //
  ...
  useEffect(() => {
    if (!shouldCopySelectionOnSelect()) {
      return
    }
  ...
  // Tab title: `⚠` waiting on approval/clarify, `⏳` busy, `✓` idle.
  const model = ui.info?.model?.replace(/^.*\//, '') ?? ''

  const marker = overlay.approval || overlay.clarify ? '⚠' : ui.busy ? '⏳' : '✓'

  const tabCwd = ui.info?.cwd

  useTerminalTitle(model ? `${marker} ${model}${tabCwd ? ` · ${shortCwd(tabCwd, 24)}` : ''}` : 'OpenDDE Harness')
  ...
  const answerClarify = useCallback(
    (answer: string) => {
      ...
      rpc<ClarifyRespondResponse>('clarify.respond', {
        ...
        if ($overlayState.get().clarify?.requestId === clarify.requestId) {
  ...
  const cwd = ui.info?.cwd || process.env.OPENDDE_HARNESS_CWD || process.cwd()
