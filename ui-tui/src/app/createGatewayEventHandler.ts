...
import { STARTUP_QUERY } from '../config/env.js'
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    if (startupPromptSubmitted || !STARTUP_QUERY) {
      return
    }
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `opendde -c` /
    // `opendde --tui` muscle memory and addresses the audit's "session
