...
import { applyProteinDesignProgress, type ProteinDesignProgressPayload } from './proteinDesignTaskStore.js'
...
interface ProteinDesignProgressEvent {
...
const dispatch = (
  state: InternalState,
  event: TurnEvent | ProteinDesignProgressEvent,
  sys?: (msg: string) => void,
  appendMessage?: (msg: Msg) => void
): void => {
  switch (event.type) {
    ...
    case 'error':
      ...
      return
    case 'protein_design.progress': {
      const changedToTerminal = applyProteinDesignProgress(event.payload)

      // Routine progress belongs to the fixed background-task bar, not the
      // conversation transcript. Keep a single terminal notification when a
      // live event explicitly crosses into completed/failed.
      if (sys && changedToTerminal) {
        const item = event.payload
        const result = item.status === 'completed' ? 'completed' : 'failed'
        const progress = item.cycle == null ? '' : ` · cycle ${item.cycle}/${item.total_cycles || '?'}`
        sys(`${result === 'completed' ? '✓' : '✗'} protein design ${item.task_id.slice(0, 8)} ${result}${progress}`)
      }
      return
    }
    case 'cron.delivered': {
