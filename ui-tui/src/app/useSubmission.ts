...
export function useSubmission(opts: UseSubmissionOptions) {
  ...
  const handleBusyInput = useCallback(
    (full: string) => {
      ...
      // the gateway is still mid-response when `turn.send` lands,
  ...
  const dispatchSubmission = useCallback(
    (full: string) => {
      ...
      if (editIdx !== null) {
        ...
        if (getUiState().busy) {
          // 'interrupt' should reach the live turn instead of silently
...
export interface UseSubmissionOptions {
   ...
   * absent or detached, submissions are refused with a "session not ready"
   * note until the handle attaches.
   */
