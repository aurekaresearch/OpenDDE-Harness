// Diagnostic color renderers for `ddeharness tui --print-colors` (flat swatches)
...
import { openddeHarnessLogo } from '../banner.js'
...
export function renderColorSwatches(theme: Theme, tier: 0 | 1 | 2 | 3): string {
  ...
  const lines: string[] = []
  lines.push(`OpenDDE Harness TUI palette — tier ${tier} (${TIER_NAMES[tier] ?? 'unknown'})`)
  lines.push('')
  ...
  lines.push('')
  lines.push('Force a tier to compare:  ddeharness tui --color <truecolor|256|16|none> --print-colors')
...
export function renderColorPreview(theme: Theme, tier: 0 | 1 | 2 | 3): string {
  ...
  out.push(`OpenDDE Harness TUI color usage preview — tier ${tier} (${TIER_NAMES[tier] ?? 'unknown'})`)

  section('Banner (yellow ramp)')
  for (const [color, text] of openddeHarnessLogo(theme.yellow)) {
  ...
  out.push('')
  out.push(fg('Force a tier:  ddeharness tui --color <truecolor|256|16|none> --preview-colors', c.muted))
