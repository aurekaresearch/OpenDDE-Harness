...
//
// Both gate on OPENDDE_HARNESS_DEV_PERF=1 and dump JSON-lines (default ~/.opendde_harness/perf.log,
// override OPENDDE_HARNESS_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// OPENDDE_HARNESS_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.OPENDDE_HARNESS_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.OPENDDE_HARNESS_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.OPENDDE_HARNESS_DEV_PERF_LOG?.trim() || join(homedir(), '.opendde_harness', 'perf.log')
