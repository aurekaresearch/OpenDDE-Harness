...
describe('completionRequestForInput', () => {
  ...
  it('does not route absolute paths through slash completion', () => {
    expect(
      completionRequestForInput('/home/d/Desktop/agenda/CrimsonRed/.opendde_harness/plans/2026-05-04-HANDOFF-NEXT.md')
    ).toMatchObject({
      method: 'complete.path',
      params: { word: '/home/d/Desktop/agenda/CrimsonRed/.opendde_harness/plans/2026-05-04-HANDOFF-NEXT.md' },
      replaceFrom: 0
    })
  })
...
const MINI_REGISTRY: SlashCommand[] = [
  { name: 'quit', aliases: ['exit', 'q'], help: 'exit opendde', run: vi.fn() },
...
describe('slashCompletions supported filter', () => {
  ...
  // The owner-reviewed curation: drop commands hidden, keep commands shown.
  const DROP_NAMES = [
    'redraw',
    'fortune',
    'terminal-setup',
