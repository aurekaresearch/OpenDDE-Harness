...
describe('applyDisplay', () => {
  ...
  it('leaves mouse tracking alone when the config does not mention it', () => {
    // Its default comes from OPENDDE_HARNESS_TUI_DISABLE_MOUSE, not from this function, and
...
describe('normalizeBusyInputMode', () => {
  ...
  it('defaults to queue for missing/unknown values (TUI-only override)', () => {
    ...
    expect(normalizeBusyInputMode('steer')).toBe('queue')
...
describe('applyDisplay → busy_input_mode', () => {
  ...
  it('threads display.busy_input_mode into $uiState', () => {
    ...
    applyDisplay({ config: { display: { busy_input_mode: 'interrupt' } } }, setBell)
...
// The live mtime-reload path shares its fetch + apply body with the initial
// hydration as ``hydrateFullConfig()``, exercised directly here.
describe('hydrateFullConfig', () => {
  ...
  it('fetches config.get full and applies the display block', async () => {
    const gw = makeFakeGw({ config: { display: { bell_on_complete: true, tui_statusbar: 'top' } } })
    const setBell = vi.fn()
  ...
  it('reapplies the latest value on each invocation (mtime-reload semantics)', async () => {
    const gw = makeFakeGw({ config: { display: { tui_statusbar: 'top' } } })
    const setBell = vi.fn()

    await hydrateFullConfig(gw, setBell)
    expect($uiState.get().statusBar).toBe('top')

    gw.request = vi.fn(() => Promise.resolve({ config: { display: { tui_statusbar: 'off' } } }))

    await hydrateFullConfig(gw, setBell)
    expect($uiState.get().statusBar).toBe('off')
  })

  it('returns null and applies defaults when the RPC fails', async () => {
    ...
    // quietRpc() swallows the error and returns null; applyDisplay's null-cfg
    // path still applies the documented bell default (false).
