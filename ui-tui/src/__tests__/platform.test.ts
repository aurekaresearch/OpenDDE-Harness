...
describe('shouldCopySelectionOnSelect', () => {
  it('enables copy-on-select for local macOS terminals', async () => {
    const { shouldCopySelectionOnSelect } = await importPlatform('darwin')

    expect(shouldCopySelectionOnSelect('darwin', {})).toBe(true)
  })

  it('enables OSC52 copy-on-select for remote Linux sessions', async () => {
    const { shouldCopySelectionOnSelect } = await importPlatform('linux')

    expect(shouldCopySelectionOnSelect('linux', { SSH_CONNECTION: '1 2 3 4' })).toBe(true)
    expect(shouldCopySelectionOnSelect('linux', { SSH_CLIENT: '1 2 3' })).toBe(true)
    expect(shouldCopySelectionOnSelect('linux', { SSH_TTY: '/dev/pts/1' })).toBe(true)
  })

  it('leaves local Linux selection behavior unchanged', async () => {
    const { shouldCopySelectionOnSelect } = await importPlatform('linux')

    expect(shouldCopySelectionOnSelect('linux', {})).toBe(false)
  })
})
