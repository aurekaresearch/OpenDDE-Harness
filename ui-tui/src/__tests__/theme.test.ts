...
// OPENDDE_HARNESS_TUI_THEME=light (or OPENDDE_HARNESS_TUI_BACKGROUND set to something
...
const RELEVANT_ENV = [
  'OPENDDE_HARNESS_TUI_LIGHT',
  'OPENDDE_HARNESS_TUI_THEME',
  'OPENDDE_HARNESS_TUI_BACKGROUND',
  'COLORFGBG',
  'COLORTERM',
  'TERM_PROGRAM'
] as const
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('OpenDDE Harness')
    expect(DEFAULT_THEME.brand.icon).toBe('ϒ')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#A78BFA')
...
describe('brand purple ramp (title-gradient-table.md)', () => {
  ...
  it('keeps the documented dark title bands', async () => {
    ...
    expect(DARK_THEME.yellow[0]).toBe('#F5F3FF') // .50
    expect(DARK_THEME.yellow[2]).toBe('#C4B5FD') // .300
    expect(DARK_THEME.yellow[3]).toBe('#A78BFA') // .500
    expect(DARK_THEME.yellow[5]).toBe('#7C3AED') // .700
    expect(DARK_THEME.yellow[6]).toBe('#5B21B6') // .900
  })

  it('gives light its own gold ramp re-derived around #6D28D9, not the dark scale', async () => {
    const { DARK_THEME, LIGHT_THEME } = await importThemeWithCleanEnv()

    expect(LIGHT_THEME.yellow[0]).toBe('#EDE9FE') // .50
    expect(LIGHT_THEME.yellow[2]).toBe('#C4B5FD') // .300
    expect(LIGHT_THEME.yellow[3]).toBe('#6D28D9') // .500
    expect(LIGHT_THEME.yellow[5]).toBe('#4C1D95') // .700
    expect(LIGHT_THEME.yellow[6]).toBe('#3B0764') // .900
  ...
  it('carries scheme-specific 256-color ramps for the documented title bands', async () => {
    ...
    // purple.300 and purple.500 must not collapse onto the same 256 index.
  ...
  it('keeps 16-color purple for both schemes', async () => {
    const { resolveTheme } = await importThemeWithCleanEnv()

    expect(resolveTheme('light', 1).yellow).toEqual(resolveTheme('dark', 1).yellow)
    expect(resolveTheme('light', 1).yellow[0]).toBe('ansi:magentaBright')
  })
})
...
describe('cursorColorHex (OSC 12 hardware cursor)', () => {
  it('returns a truecolor hex even when the theme tier is 256/16 (default dark)', async () => {
    ...
    expect(cursorColorHex(resolveTheme('dark', 3))).toBe('#A78BFA') // truecolor primary passed through
    expect(cursorColorHex(resolveTheme('dark', 2))).toBe('#A78BFA')
    expect(cursorColorHex(resolveTheme('dark', 1))).toBe('#A78BFA')
  })

  it('uses the light title color across all tiers when the scheme is light', async () => {
    const { cursorColorHex, resolveTheme } = await importThemeWithEnv({ OPENDDE_HARNESS_TUI_THEME: 'light' })

    expect(cursorColorHex(resolveTheme('light', 3))).toBe('#6D28D9')
    expect(cursorColorHex(resolveTheme('light', 2))).toBe('#6D28D9')
    expect(cursorColorHex(resolveTheme('light', 1))).toBe('#6D28D9')
  })
})
...
describe('detectLightMode', () => {
  ...
  it('honors OPENDDE_HARNESS_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ OPENDDE_HARNESS_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets OPENDDE_HARNESS_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', OPENDDE_HARNESS_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors OPENDDE_HARNESS_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ OPENDDE_HARNESS_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', OPENDDE_HARNESS_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', OPENDDE_HARNESS_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses OPENDDE_HARNESS_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ OPENDDE_HARNESS_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('applyDetectedBackground (OSC 11 reply)', () => {
  ...
  it('caches the measured color into OPENDDE_HARNESS_TUI_BACKGROUND', async () => {
    const { applyDetectedBackground } = await importThemeWithCleanEnv()

    applyDetectedBackground('rgb:eaea/eaea/eaea')

    expect(process.env.OPENDDE_HARNESS_TUI_BACKGROUND).toBe('#eaeaea')
  })

  it('lets an explicit OPENDDE_HARNESS_TUI_THEME override the measured background', async () => {
    const { applyDetectedBackground, currentScheme } = await importThemeWithEnv({ OPENDDE_HARNESS_TUI_THEME: 'dark' })
...
describe('fromSkin', () => {
  ...
  // OPENDDE_HARNESS_TUI_THEME=light would flip the base palette and make these
...
describe('resolveTheme', () => {
  ...
  it('uses curated ansi256 values at tier 2', async () => {
    ...
    expect(resolveTheme('dark', 2).color.primary).toBe('ansi256(141)')
    expect(resolveTheme('light', 2).color.primary).toBe('ansi256(91)')
  })

  it('uses named 16-color values at tier 1', async () => {
    const { resolveTheme } = await importThemeWithCleanEnv()

    expect(resolveTheme('dark', 1).color.primary).toBe('ansi:magentaBright')
    expect(resolveTheme('dark', 1).color.accent).toBe('ansi:magentaBright')
  })
