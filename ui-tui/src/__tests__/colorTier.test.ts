...
describe('parseColorOverride', () => {
  it('maps named + numeric tiers', () => {
    ...
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: '24bit' })).toBe(3)
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: '256' })).toBe(2)
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: '16' })).toBe(1)
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: 'none' })).toBe(0)
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: '3' })).toBe(3)
  })
  ...
  it('treats auto / unset / unknown as auto (null)', () => {
    ...
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: 'wat' })).toBeNull()
  })

  it('honors the legacy OPENDDE_HARNESS_TUI_TRUECOLOR alias', () => {
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_TRUECOLOR: '1' })).toBe(3)
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_TRUECOLOR: '0' })).toBeNull()
  })

  it('lets OPENDDE_HARNESS_TUI_COLOR win over the legacy alias', () => {
    expect(parseColorOverride({ OPENDDE_HARNESS_TUI_COLOR: '256', OPENDDE_HARNESS_TUI_TRUECOLOR: '1' })).toBe(2)
  })
})

describe('applyColorOverride', () => {
  it('pins HERMES_TUI_LEVEL to the requested tier', () => {
    const env: NodeJS.ProcessEnv = { OPENDDE_HARNESS_TUI_COLOR: '256' }
    expect(applyColorOverride(env)).toBe(2)
    expect(env.HERMES_TUI_LEVEL).toBe('2')
  })

  it('forces colors off for none', () => {
    const env: NodeJS.ProcessEnv = { OPENDDE_HARNESS_TUI_COLOR: 'none' }
  ...
  it('NO_COLOR beats a typed --color (forces off)', () => {
    const env: NodeJS.ProcessEnv = { NO_COLOR: '1', OPENDDE_HARNESS_TUI_COLOR: 'truecolor' }
    expect(applyColorOverride(env)).toBe(0)
    expect(env.HERMES_TUI_LEVEL).toBe('0')
  })

  it('pins an explicit truecolor downgrade target regardless of FORCE_COLOR', () => {
    const env: NodeJS.ProcessEnv = { FORCE_COLOR: '3', OPENDDE_HARNESS_TUI_COLOR: '256' }
    expect(applyColorOverride(env)).toBe(2)
    expect(env.HERMES_TUI_LEVEL).toBe('2')
  })
})
