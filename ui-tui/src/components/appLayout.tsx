...
import { $hasPaneOverlay, $isBlocked, $overlayState, patchOverlayState } from '../app/overlayStore.js'
...
import { FloatingOverlays, OverlayPane, PromptZone } from './appOverlays.js'
        ...
        <Box flexDirection="column" paddingX={1}>
          ...
          {transcript.virtualRows.slice(transcript.virtualHistory.start, transcript.virtualHistory.end).map(row => (
            <Box flexDirection="column" key={row.key} ref={transcript.virtualHistory.measureRef(row.key)}>
              ...
              {row.msg.kind === 'intro' ? (
                // The panel renders its lockup before session.info arrives and
                // only fills its sections in afterwards, so the welcome screen
                // never re-lays out.
                <Box flexDirection="column" paddingTop={1}>
                  <SessionPanel info={row.msg.info ?? null} sid={ui.sid} t={ui.theme} />
                </Box>
...
const ComposerPane = memo(function ComposerPane({
  ...
  return (
    ...
    >
      ...
      <BackgroundTaskBar cols={composer.cols} t={ui.theme} />
      ...
      <Box
        ...
        position="relative"
      >
        <FloatingOverlays cols={composer.cols} compIdx={composer.compIdx} completions={composer.completions} />
...
// The pickers, the skills hub and the pager take the transcript area over
// while open: floated above the composer they overlapped the welcome panel,
// which showed around and behind them.
const PaneOverlayPane = memo(function PaneOverlayPane({
  actions,
  composer
}: Pick<AppLayoutProps, 'actions' | 'composer'>) {
  return (
    <OverlayPane
  ...
  return (
    <Box marginTop={at === 'top' ? 1 : 0}>
      <StatusRule
        ...
        updateCommand={ui.info?.update_command || 'ddeharness upgrade'}
...
export const AppLayout = memo(function AppLayout({
  ...
  const overlay = useStore($overlayState)
  const paneOverlay = useStore($hasPaneOverlay)
  const ui = useStore($uiState)
      ...
      <Box flexDirection="column" flexGrow={1}>
        <Box flexDirection="row" flexGrow={1}>
          {overlay.agents ? (
            ...
            <PerfPane id="overlay">
              <PaneOverlayPane actions={actions} composer={composer} />
            </PerfPane>
          ) : (
