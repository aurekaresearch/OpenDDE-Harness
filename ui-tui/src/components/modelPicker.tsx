...
function unconfiguredWarning(p: ModelOptionProvider): string {
  if (p.auth_type === 'oauth') {
    return `run \`ddeharness provider login ${p.slug.replace(/_/g, '-')}\``
  }
...
export function ModelPicker({ gw, launcher, onCancel, onSelect, sessionId, suspend, t }: ModelPickerProps) {
  ...
  const signIn = async (target: { name: string; slug: string }) => {
    ...
    if (result.code !== 0) {
      setLoginError(`\`opendde ${args.join(' ')}\` exited with code ${result.code}`)
      setLoginPhase('idle')
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Saved to ~/.opendde_harness/.env{showBase ? ' · Tab switches field' : ''}
        </Text>
  ...
  if (stage === 'oauthLogin' && loginTarget) {
    const command = `ddeharness provider login ${loginTarget.slug.replace(/_/g, '-')}`
    return (
      <Box flexDirection="column" width={width}>
        <Text bold color={t.color.accent} wrap="truncate-end">
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          This is a browser sign-in. OpenDDE Harness hands the terminal over until it ends:
        </Text>
        ...
        {loginError ? (
          ...
          <Text color={t.color.label} wrap="truncate-end">
            sign-in ended, but OpenDDE Harness still finds no credentials for it.
          </Text>
        ) : loginPhase === 'done' ? (
          <Text color={t.color.muted} wrap="truncate-end">
            signed in.
  ...
  return (
    <Box flexDirection="column" width={width}>
      ...
      {Array.from({ length: VISIBLE }, (_, i) => {
        ...
        const shown = bareModelId(provider, row)
        const label = provider?.model_labels?.[row]?.label
        const text = label && label !== shown ? `${label} · ${shown}` : shown
