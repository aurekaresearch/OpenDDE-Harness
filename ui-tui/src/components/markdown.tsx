...
// One memoized row per code line so a streaming fence (no stable block
// boundary inside it, so the whole block re-parses per delta) only
// re-highlights and re-renders the line that actually changed.
const HighlightedLine = memo(function HighlightedLine({ lang, line, t }: { lang: string; line: string; t: Theme }) {
  return (
    <Text>
      {highlightLine(line, lang, t).map(([color, text], kk) =>
        color ? (
      ...
      if (fence) {
        ...
        nodes.push(
          <Box flexDirection="column" key={key} paddingLeft={2}>
            ...
            {block.map((l, j) => {
              if (highlighted) {
                return <HighlightedLine key={j} lang={lang} line={l} t={t} />
              }
