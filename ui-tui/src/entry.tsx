...
// Python (see opendde_harness/cli/tui_commands.py), not via a direct ./entry.js call.
...
// Must be first import. Applies any OPENDDE_HARNESS_TUI_COLOR / NO_COLOR override by
...
// `ddeharness tui --print-colors` is a no-IPC diagnostic: dump the resolved
// palette as swatches and exit. Runs before the TTY guard so it works when
// piped, and honors any --color / OPENDDE_HARNESS_TUI_COLOR override.
if (process.env.OPENDDE_HARNESS_TUI_PRINT_COLORS === '1') {
  process.stdout.write(renderColorSwatches(DEFAULT_THEME, activeColorTier()))
  process.exit(0)
}

// `ddeharness tui --preview-colors` renders the tokens in their real UI contexts
...
if (process.env.OPENDDE_HARNESS_TUI_COLOR_PREVIEW === '1') {
...
// `ddeharness tui --check` is a no-IPC smoke path: import chain + terminal
...
// socket, so the historical OPENDDE_HARNESS_TUI_CHECK handler at the bottom of
// this file is unreachable for the check path (gw.start() rejects first
// when there's no socket, and the OPENDDE_HARNESS_RPC_SOCKET guard below would
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `opendde-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write('opendde-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  onHigh: (snap, dump) => process.stderr.write(dumpNotice(snap, dump))
})

if (process.env.OPENDDE_HARNESS_HEAPDUMP_ON_START === '1') {
...
// `ddeharness tui --check` sets OPENDDE_HARNESS_TUI_CHECK=1 and expects us to boot
...
// failure signal `ddeharness tui --check` is designed to surface.
if (process.env.OPENDDE_HARNESS_TUI_CHECK === '1') {
  setTimeout(() => {
    try {
      void gw.kill()
