...
const UNRECOGNIZED_ESCAPE_RE = /^\u001B[[\]OP_]./
...
function parseKey(keypress: ParsedKey): [Key, string] {
  ...
  // A CSI/OSC/DCS/APC/SS3 sequence that no parser recognized is a terminal
  // report (a truncated mouse report or query reply that outlived the
  // tokenizer's hold), not text: never let its parameter bytes reach an input.
  if (!keypress.name && !keypress.isPasted && UNRECOGNIZED_ESCAPE_RE.test(keypress.sequence ?? '')) {
    input = ''
  }
