  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // OPENDDE_HARNESS_TUI_DEBUG_TERMINFO=1. Useful when modifier+enter behavior
        // regresses on a new terminal/transport stack — set the env var,
        // launch the TUI, then inspect ~/.opendde_harness/logs/tui-node-diag.log.
        if (process.env.OPENDDE_HARNESS_TUI_DEBUG_TERMINFO === '1') {
          try {
            const diagPath = join(homedir(), '.opendde_harness', 'logs', 'tui-node-diag.log')
