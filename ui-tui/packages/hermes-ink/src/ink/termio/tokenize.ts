...
export function createTokenizer(options?: TokenizerOptions): Tokenizer {
  ...
  return {
    ...
    flush(): Token[] {
      // A CSI/OSC/DCS/APC sequence that already carries payload bytes is a
      // terminal report cut at a stdin chunk boundary (e.g. `ESC [ < 64 ; 100 ;`
      // with `23M` still in the kernel buffer), never something a human typed
      // within the flush window. Hold it across one flush so the next chunk can
      // complete it; emitting it truncated would decode as text downstream.
      if (!heldFlush && isPayloadSequence(currentState, currentBuffer)) {
        heldFlush = true
...
const isPayloadSequence = (state: State, buffer: string): boolean =>
  (state === 'csi' || state === 'osc' || state === 'dcs' || state === 'apc') && buffer.length > 2
