...
describe('escape sequences split across stdin chunks', () => {
  it('holds a CSI cut mid-parameters across the watchdog flush and completes it from the next chunk', () => {
    let [keys, state] = parseMultipleKeypresses(INITIAL_STATE, 'hi\x1b[<64;100;')

    expect(keys).toEqual([expect.objectContaining({ kind: 'key', sequence: 'hi' })])
    expect(state.incomplete).toBe('\x1b[<64;100;')

    // App's 50ms flush timer fires before the poll phase delivers the tail.
    ;[keys, state] = parseMultipleKeypresses(state, null)

    expect(keys).toEqual([])
    expect(state.incomplete).toBe('\x1b[<64;100;')
    ;[keys, state] = parseMultipleKeypresses(state, '23M')

    expect(keys).toEqual([expect.objectContaining({ kind: 'key', name: 'wheelup', sequence: '\x1b[<64;100;23M' })])
    expect(state.incomplete).toBe('')
  })

  it('keeps typed text and mouse reports apart when a report straddles a chunk boundary', () => {
    const chunks = ['ab\x1b[<65;10;5M\x1b[<35;1', '1;5Mcd\x1b[<0;3;', '4M\x1b[<0;3;4mef']
    const events = []
    ...
    for (const chunk of chunks) {
      ...
      events.push(...keys)
      // The watchdog flush lands between chunks whenever a render blocks the loop.
      const [flushed, afterFlush] = parseMultipleKeypresses(next, null)
      events.push(...flushed)
      state = afterFlush
    }

    expect(events.map(e => (e.kind === 'key' ? e.name || e.sequence : `${e.kind}:${e.button}`))).toEqual([
      'ab',
      'wheeldown',
      'mouse:35',
      'cd',
  ...
  it('still flushes a lone ESC as the escape key', () => {
    let [keys, state] = parseMultipleKeypresses(INITIAL_STATE, '\x1b')

    expect(keys).toEqual([])
    ;[keys, state] = parseMultipleKeypresses(state, null)

    expect(keys).toEqual([expect.objectContaining({ name: 'escape' })])
    expect(state.incomplete).toBe('')
  })

  it('gives a truncated sequence up on the second flush so a dead tail cannot wedge input', () => {
    let [keys, state] = parseMultipleKeypresses(INITIAL_STATE, '\x1b]11;rgb:00')
    ...
    expect(keys).toEqual([expect.objectContaining({ kind: 'key', name: '', sequence: '\x1b]11;rgb:00' })])
    expect(state.incomplete).toBe('')
  })
})
