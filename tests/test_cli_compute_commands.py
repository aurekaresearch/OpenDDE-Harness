...
def prepared(monkeypatch):
    ...
    monkeypatch.setattr(
        compute_code, "prepare_runtime_code", lambda cache, **kwargs: calls.setdefault("code", (cache, kwargs))
    )
    return calls
                ...
                "config": {
                    "protein-design": {
                        ...
                        "compute_docker": {
                            ...
                            "opendde_checkpoint": str(tmp_path / "checkpoint/opendde_abag.pt"),
                        },
                    }
                }
            }
...
def test_unknown_checkpoint_rejected_in_local_mode(app, prepared, monkeypatch, tmp_path):
    ...
    result = CliRunner().invoke(
        app, ["compute", "prepare", "--root", str(tmp_path), "--mode", "local", "--checkpoint", "x.pt"]
    )
...
def test_preparation_errors_exit_one_with_message(app, monkeypatch, tmp_path):
    monkeypatch.setattr(onboard_commands, "_load_raw_config", lambda: {})
    monkeypatch.setattr(
        compute_assets, "prepare", lambda *a, **k: (_ for _ in ()).throw(ValueError("curl is required"))
    )
...
def test_prepare_help_lists_no_doctor_flags(app):
    ...
    assert "--verify-hashes" not in output and "--fix" not in output
    assert "--sources-only" in output
...
def test_compute_help_lists_prepare_serve_and_stop(app):
    ...
    for command in ("prepare", "serve", "stop"):
        assert f" {command} " in output or f" {command}\n" in output
...
def local_state(monkeypatch):
    ...
    monkeypatch.setattr(
        onboard_commands,
        "_load_raw_config",
        lambda: {"plugins": {"config": {"protein-design": {"compute_token": "tok"}}}},
    )
