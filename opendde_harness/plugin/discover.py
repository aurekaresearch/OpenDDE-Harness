...
_MANIFEST_FILENAME = "opendde-harness-plugin.toml"
...
class PluginDiscovery:
    ...
    def _scan_dir(
        ...
        """Recursively find plugin manifests below ``root``.

        Supporting nested category directories lets bundled plugins express
        ownership without forcing unrelated tool plugins under a memory-only
        namespace. The manifest's parent directory remains informational;
        the canonical plugin id is the one declared in TOML.
        """
        out: list[DiscoveredPlugin] = []
        if not root.is_dir():
            return out
        for manifest_path in sorted(root.rglob(_MANIFEST_FILENAME)):
            sub = manifest_path.parent
            try:
    ...
    def _scan_entry_points(self, group: str) -> list[DiscoveredPlugin]:
        ...
        ``opendde_harness_mem0``). We use ``importlib.resources`` to locate
        ``opendde-harness-plugin.toml`` inside that package. The package's
