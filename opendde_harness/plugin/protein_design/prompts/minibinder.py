"""Text-only prompts for non-antibody mini binder design and optimization."""

COMMON = """You design and optimize a non-antibody mini binder against a protein target.
An X is an unknown residue placeholder, not a foldable amino acid. Use full redesign to materialize masked seeds before folding.
...
INSTRUCTIONS = {
    ...
    "design": "Select exactly one allowed skill_id. For full redesign assign every mutable position exactly once using canonical amino acids, including unchanged residues; the point-mutation budget does not apply. For point mutation emit distinct candidates with candidate_id and mutations (chain_id, position, from_aa, to_aa). Respect the mutation budget. For inverse folding return the anchor/control proposal described in its skill. Consider foldability, target interface and earlier outcomes; never modify the target. applied_learned_skill_ids must refer only to retrieved advisory skills.",
