...
fields (``response_modifier`` / ``on_user_inbound`` /
``decision_consumer``).
...
2. **Async everywhere.** Even phases that look synchronous today (e.g.
   ...
   here so an LLM-based hook can be added without redoing the
   interface. Sync callers can simply
   ``return`` immediately.
...
3. **Three orthogonal return modes**, all expressed via
   ...
   - *short_circuit_result* — halt the chain and the AgentLoop returns
     ...
     answer, depending on the phase). Used by the
     ``decision_consumer`` short-circuit.
...
class AgentHook(ABC):
    ...
    def name(self) -> str:
        ...
        Subclasses should override for clarity, so the source subsystem
    ...
    async def before_user_inbound(self, ctx: AgentHookContext) -> HookDecision:
        ...
        Used for:
          - a ``decision_consumer`` hook returning a short-circuit reply
            when it recognizes and claims the inbound;
          - observer hooks that record engagement without
            short-circuiting.
    ...
    async def after_send(self, ctx: AgentHookContext) -> HookDecision:
        ...
        Used for:
          - a hook that appends to the outbound reply (via
            ``modified_content``);
          - observer hooks that record the final exchange.
