...
class QuestionBroker:
    ...
    def cancel(self, key: str) -> bool:
        conversation_id = key if key in self._pending else self._by_request.get(key)
        pending = self._pending.get(conversation_id) if conversation_id is not None else None
        return self.reply(key, pending.default) if pending is not None else False
