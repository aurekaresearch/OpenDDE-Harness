...
async def _default_session_info(
    ...
    # Nudge the status bar to run `ddeharness upgrade` when the cached latest release
    # is newer. Reading the cache is pure/fast; the cache is refreshed once per
    # launch from the `ddeharness tui` entrypoint (see cli/tui_commands.py), so a
    # freshly published release shows up on the next launch.
    notice = update_notice(_OPENDDE_HARNESS_VERSION)
