...
2. **S2 dispatch-compat**: ``_is_dispatch_compatible`` reflects
   ``opendde_harness.cli.commands.app`` (Typer 0.20+) — any registered command
   ...
   blocked by the blacklist (``provider login``, the long-running
   ``compute serve``) + recursive ``tui`` / ``onboard`` wizard.
