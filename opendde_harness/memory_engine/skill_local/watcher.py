...
memory evolver for its own writes.
...
_LIVE_WATCHERS: weakref.WeakSet[SkillFileWatcher] = weakref.WeakSet()


def _stop_live_watchers() -> None:
    """Stop Rust-backed watchers before interpreter module teardown."""
    for watcher in list(_LIVE_WATCHERS):
        watcher.stop()


atexit.register(_stop_live_watchers)
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            log.info(
                ...
                "disabled. Reinstall opendde (or run "
