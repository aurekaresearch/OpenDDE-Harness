...
from opendde_harness.utils.helpers import ensure_dir, estimate_message_tokens, estimate_prompt_tokens_chain
...
class MemoryStore:
    ...
    def __init__(
        ...
        # that write MEMORY.md so every writer serializes on the same
        # advisory lock.
    ...
    def _fcntl_locked(self, lock_path: Path) -> Iterator[None]:
        ...
        # concurrent writes to MEMORY.md.
    ...
    # MemoryStore is the single owner of MEMORY.md / HISTORY.md mutation
    # logic; direct callers import it, with no Protocol indirection.
    ...
    def update_section(
        ...
        *,
        ...
        updates under one lock when needed::
            ...
            with store.locked():
                store.update_section("## Observations", body)

        ``at_end=True`` (default) routes through the existing
        :func:`_splice_h2_section_at_end` helper which guarantees the
        named section ends at file-end after the write, for writers
        where heading order is meaningful. ``at_end=False`` preserves the existing position
