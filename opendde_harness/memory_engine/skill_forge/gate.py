...
Ported from the pre-memory-integration
...
_GATE_LOG_PATH_ENV = "OPENDDE_HARNESS_GATE_LOG_PATH"
...
class LLMGateFilter:
    ...
    def set_provider(self, provider: "LLMProvider", model: str) -> None:
        ...
        ``_model`` is only ever the operator's explicit ``llm_gate_model``
        pin, so it survives the switch on purpose; unset, the gate follows
        whatever the new provider serves by default. A pin is only a model
        id -- the credential comes from the provider -- so a pin naming a
        vendor the provider does not serve is broken at boot and stays
        broken here.
        """
        del model
        self._provider = provider
    ...
    def _build_prompt(
        ...
        # Verbatim port of the pre-memory-integration
