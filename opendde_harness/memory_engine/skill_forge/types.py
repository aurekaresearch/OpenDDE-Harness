...
- :class:`RouterHit` is **self-contained**. Unlike the legacy
  ``ScoredSkill`` in :mod:`opendde_harness.memory_engine.skill_local.types` (which
...
- :class:`SkillSource` is **internal**. Per the design decision
  ...
  Memory) rather than exposed as a plugin contribution point.
...
class RouterHit:
    ...
    ``"memory/abc123"``. The slash split is unambiguous because
...
class SkillSource(Protocol):
    ...
    name: str
    """Stable source identifier (``"local"`` / ``"mass"`` / ``"memory"``).
    ...
    async def search(
        ...
        ranker (Memory can feed it into ``backend.recall`` if the
