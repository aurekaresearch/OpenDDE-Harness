"""MemorySkillSource — re-emit ``MemoryBackend`` agent-track hits as RouterHits.
...
with the ``memory/`` prefix so :class:`SkillForgeRouter` can RRF-fuse them
against Local + Mass.

The source is **host code, not part of any plugin**. The actual
backend behind it can be the bundled long-term memory plugin or any other
...
class MemorySkillSource:
    ...
    async def search(
        ...
        for m in hits:
            ...
            out.append(
                RouterHit(
                    qualified_id=f"memory/{native_id}",
                    name=name,
                    content=m.text,
                    score=m.score,
                    meta={
