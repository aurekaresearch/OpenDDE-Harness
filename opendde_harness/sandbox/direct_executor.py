...
from opendde_harness.sandbox.interfaces import ExecResult, SandboxExecutor
...
class DirectExecutor(SandboxExecutor):
    ...
    async def exec(
        ...
        try:
            ...
            process.kill()
            try:
                # Reap the child *and* drain/close both PIPE transports.  A
                # bare ``wait()`` leaves the cancelled ``communicate()`` pipe
                # readers alive until GC, which leaks descriptors in a
                # long-running TUI and raises during interpreter shutdown.
                await asyncio.wait_for(process.communicate(), timeout=5.0)
            except asyncio.TimeoutError:
                # Last-resort close for a broken child transport.  This is an
                # asyncio subprocess implementation detail, used only after a
                # killed process still failed to reap within the grace period.
