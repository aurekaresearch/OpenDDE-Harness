"""``opendde session`` subapp — manage conversation sessions.
...
- Untitled sessions are auto-named from their first user message (see
  ``opendde_harness.session.manager``); ``--title`` names the session up front
...
def session_create(
    ...
    if title is not None:
        ...
        console.print(f"  Session key: {key}")
    else:
        console.print(chat_id)
        console.print(f"[dim]  (session key: {key})[/dim]")
...
def session_resume(
    ...
    The printed key is the one the TUI resumes; this command is a pure
    resolver and does not start an agent loop.
    """
    manager = _open_manager()
...
def session_fork(
    ...
    the new child id (first line) so the TUI can resume it.
    """
    manager = _open_manager()
    key = resolve_session(manager, id_or_prefix)
    ...
    console.print(f"[dim]  (forked from {_bare_id(key)}; session key: {child.key})[/dim]")
