"""``opendde plugins`` — inspect installed memory / context plugins.

Reads ``OpenDDEHarnessConfig.plugins`` + the live :class:`PluginRegistry`
...
def register(app: typer.Typer) -> None:
    ...
    def plugins(
        config_path: Optional[str] = typer.Option(
            ...
            "--config",
            "-c",
            help="Path to config file (default: ~/.opendde_harness/config.json)",
        ),
        ...
        # Import lazily so ``ddeharness --help`` doesn't pay for plugin
        # discovery on every invocation.
        from opendde_harness.cli._plugin_stack import plugin_discovery_sources
...
def _load_ec_config(config_path: str | None):
    """Load OpenDDEHarnessConfig with the same fallback the other CLI
    ...
    # opendde loader. ``load_runtime_config`` is invoked for parity
    # with other CLI commands (sets ``set_config_path`` so downstream
    # readers see the same file).
    load_runtime_config(config_path)
    return load_opendde_harness_config(
...
def _render_plugin_table(
    ...
    *,
    ...
    if not discovered:
        console.print(
            "[yellow]No plugins discovered.[/yellow] The long-term memory backend "
            "ships bundled — run [bold]uv sync[/bold] — or drop a manifest "
            "under [bold]~/.opendde_harness/plugin/[/bold].",
        )
        return

    table = Table(
        title="OpenDDE Harness plugins",
        show_header=True,
...
def _source_label(source) -> str:
    ...
    from opendde_harness.plugin import Source
