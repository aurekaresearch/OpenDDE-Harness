...
function renderMemoryFeedbackCard(span) {
  ...
  const outText = 'No output — backend.feedback is a deliberate no-op in the memory backend: the skill-usage signal is captured but not consumed.';
  return memoryIoCard('Feedback · Input', '', preBody(inText)) + memoryIoCard('Feedback · Output', 'memory no-op', preBody(outText));
}
...
function renderProteinDesignWorkspace() {
  ...
  if (workspace && run && workspace._proteinRunId === run.taskId && (
  ...
  const traceAvailable = run
    ? (state.data?.sessions || []).some(
        (session) =>
          ...
          (session.traces || []).some((trace) => trace.traceId === run.traceId)
      )
    : false;
...
async function loadProteinDesignRun(
  taskId,
  runs = state.proteinDesignData?.runs || [],
...
async function loadProteinDesignData(renderOnComplete = true) {
  ...
  try {
    ...
    const selectionStillAvailable = (summary.runs || []).some(
      (run) => run.taskId === state.selectedProteinDesignRunId
    );
...
try { savedTheme = localStorage.getItem('tracing:theme') || 'light'; } catch {}
applyTheme(savedTheme);
themeButton.addEventListener('click', () => {
  const theme = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark';
  applyTheme(theme);
  try { localStorage.setItem('tracing:theme', theme); } catch {}
});
