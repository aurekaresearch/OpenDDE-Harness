...
OpenDDE Harness AI-collaboration spec. **Read this file before making any code change in this repo.**
...
| 6 | [Repository assets](#6-repository-assets) | No report assets, web artifacts, or large files in PRs |
...
| ✅ Good | ❌ Bad |
|---|---|
| `feat/esm_guided_mutation` | `feat/优化` |
| `fix/fold_backend_device_pick` | `bugfix` |
| `refactor/cli_compute_assets` | `huangjie-test` |
...
**scope** — a top-level subpackage of `opendde_harness/`. See the `Architecture & Structure` section of `README.md` for the canonical list. Spanning multiple scopes → omit the scope, or use `(*)`.
...
```
feat(cli): rename compute prepare flags to --assets and --weights
fix(plugin): keep post-filter ranking when a candidate fails scoring
refactor(cli): fold doctor asset checks into compute prepare
feat(plugin): add FoldMason tree backend
```

❌ Bad:
- `更新代码` (non-English + no type/scope);
- `update` (no type/scope);
- `feat: Compute 命令重命名.` (uppercase + period + non-English + no scope).
...
- one file per module (aligns with `opendde_harness/cli/<module>_commands.py`);
...
| ❌ Wrong | ✅ Right | Reason |
|---|---|---|
| `test_cli_doctor.py` | `test_cli_doctor_commands.py` | missing `_commands` suffix |
| `test_doctor_cli.py` | `test_cli_doctor_commands.py` | order reversed |
| `test_cli_doctor_phase4.py` | merge into `test_cli_doctor_commands.py` | no phase suffix |
